def solution(A, B):
    ...
    each other. Then only one fish can stay alive − the larger fish eats the smaller one.
    ...
    for idx, fish in enumerate(A):
        if B[idx] == 1:
            # fish heads on downstream
            fish_dn.append(fish)

        else:
            # fish heads on upstream and has to fight with all fish
            # from fish_dn one by one:
            while fish_dn:
                vice_fish = fish_dn[-1]
                if fish > vice_fish:
                    fish_dn.pop() # win so far
                else:
                    break # lose and die

            else: # no fish encountered
                alive_cnt +=1

    # Now alive_cnt contains all alive fish from upstream team
    # We have to add alive fish from downstream team:
    alive_cnt += len(fish_dn)
    return alive_cnt
